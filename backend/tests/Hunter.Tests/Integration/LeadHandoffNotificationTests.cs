...
// comportamiento de NotifyAssigneeAsync: cuándo notifica, cuándo no, y que un fallo del
// proveedor de mensajería (red o config) nunca debe tumbar el procesamiento del webhook.
public class LeadHandoffNotificationTests
{
    private static async Task<(int OrgId, int UserId)> SeedOrgWithSellerAsync(string dbName, string? sellerPhone)
    {
        ...
        var seller = new User
        {
            ...
            PasswordHash = "irrelevant", IsActive = true, Area = UserArea.Ventas, Phone = sellerPhone
        };
        db.Users.Add(seller);
        await db.SaveChangesAsync();
    ...
    public async Task NewLead_AssigneeWithPhone_SendsHandoffSummary()
    {
        ...
        var service = new InboundMessageService(db, new KeywordIntentClassifier(), provider, NullLogger<InboundMessageService>.Instance);
    ...
    public async Task SecondInboundMessage_LeadReused_NotifiesOnlyOnce()
    {
        ...
        {
            var service1 = new InboundMessageService(db1, new KeywordIntentClassifier(), provider, NullLogger<InboundMessageService>.Instance);
        ...
        {
            var service2 = new InboundMessageService(db2, new KeywordIntentClassifier(), provider, NullLogger<InboundMessageService>.Instance);
