...
public class InterestDetectionTests
{
    ...
    public async Task Interested_Response_Above_Threshold_Creates_Lead_And_Assigns_Seller()
    {
        ...
        var service = new InboundMessageService(db, new KeywordIntentClassifier(), new StubMessageProvider(NullLogger<StubMessageProvider>.Instance));
    ...
    public async Task Same_ExternalInboundId_Processed_Twice_Does_Not_Duplicate_Lead()
    {
        ...
        {
            var result1 = await new InboundMessageService(db1, new KeywordIntentClassifier(), new StubMessageProvider(NullLogger<StubMessageProvider>.Instance)).ProcessAsync(request);
        ...
        {
            var result2 = await new InboundMessageService(db2, new KeywordIntentClassifier(), new StubMessageProvider(NullLogger<StubMessageProvider>.Instance)).ProcessAsync(request);
    ...
    public async Task Stop_Response_Creates_Suppression_And_Blocks_Prospect()
    {
        ...
        await using var db = TestDb.Create(dbName, organizationId: null);
        var result = await new InboundMessageService(db, new KeywordIntentClassifier(), new StubMessageProvider(NullLogger<StubMessageProvider>.Instance))
